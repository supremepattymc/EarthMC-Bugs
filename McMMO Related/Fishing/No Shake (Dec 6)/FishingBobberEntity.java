/**
 * COMMENTS WRITTEN BY SUPREMEPATTY (MC USERNAME: PATTYG314)
 * 
 * This is the class FishingBobberEntity.java, in the Minecraft
 * source code. It deals with how fishing bobs behave in the game,
 * and thus is critical to the recent likely bug in EarthMC where
 * shake does not work. I believe that this issue is caused by the
 * recent patch where you can no longer hook players in claims with
 ...
 * on /fishing (it was 6% for me, which would clearly be wrong in this case)
 */
...
public class FishingBobberEntity extends Entity {
   ...
   // This variable is the entity being hooked (i.e. a player or a mob used for shake)
   ...
   /**
    * This function (handleHookRetraction) handles the process of "retracting"
    * the fishing rod.
    */
   public int handleHookRetraction(ItemStack p_146034_1_) {
      if (!this.world.isRemote && this.angler != null) {
         ...
         /*************************ONLY THIS BLOCK OF CODE MATTERS IN THIS CONTEXT **********************************/
         /**
          * bringInHookedEntity is called here, which means this block is where cases where entities are hooked in
          * is handled. This code is likely not where the issue lies, but it's important to understand that this
          * code exists, so that the effects of overriding bringInHookedEntity() are understood correctly, as it
          * may have been with the McMMO plugin/fishing exploit patch.
          */
         if (this.caughtEntity != null) {
   ...
   /**
    * This function (handleStatusUpdate) is extremely important. It checks to see if 
    * the entity caught is a player or not. Although I am unaware of how McMMO calls 
    * all these functions, this one specifically checks to see if the entity caught is
    * a player, so it could play some kind of role in the code.
    */
   @OnlyIn(Dist.CLIENT)
   ...
   /**
    * I believe that this function (bringInHookedEntity()) is most likely where the problem lies.
    * 
    * The line that starts with "Vec3d vec3d" concerns the X/Y/Z aspect of the mob or player
    * being caught. We don't need to worry about this line, I believe it is fine.
    * 
    * The next line, "this.caughtEntity..." is where the problem would be. This line calls a 
    * function called "setMotion" from the entity class, which is what allows for the entity
    * to be moved by the fishing rod. From my testing on the server, it is impossible to move
    * both other players and mobs using fishing rods. While not being able to move other players
    * is (correctly) intentional, it is certainly possible that whatever patch that was implemented
    * caused this line to not function correctly. 
    * 
    * When the "shake" function works correctly on McMMO, the entity moves and takes damage. In
    * order for said entity to move, the last line with setMotion as an instance of the entity
    * caughtEntity (see line 75) would have to function, but, if the patch meant overriding this
    * function and disabling setMotion while also implementing the McMMO shake ability, then the
    * shake ability may not be able to function properly.
    * 
    * See lines 391 and 458 for more information on where this function is called
    */
